...
| **Android backup must never restore the identity** (native 0.1.20: `allowBackup` was true with empty rules, so Auto Backup restored the ORPHANED swap_addr/swap_pk on reinstall — the halt survived uninstall + node reset + reinstall — and shipped the `secrets` table off-device; now allowBackup=false with explicit excludes in both backup_rules.xml and data_extraction_rules.xml incl. <device-transfer>) | ✖ n/a — no Android backup in play: a MiniDapp reinstall gets a fresh minidapp uid and therefore a fresh SQL store, so `swap_identity` in ax_kv cannot survive a reinstall. The user-facing half (say plainly how to force a clean reset) is covered by the mismatchCard text | ✖ |
| **Command-injection guards (MA-19)** — Minima node commands are flat space-separated key:value strings, so a space in a peer/on-chain value would inject an extra parameter into a fund-moving command. Reject non-hex/non-decimal at every interpolated value in lock/lockFromCoins/claim/refund + the state-filter scans. `reqToken` accepts hex OR the literal `minima` (the mxUSDT counter-leg marker) — native **0.1.35** fixed a regression where an over-strict hex guard rejected EVERY mxUSDT counter-leg lock (broke all ERC20→mxUSDT buys); this mirror whitelists `minima` from the start. Order ingest blanks a non-hex mpk/eth (the Ed25519 signature proves authorship, not well-formedness) | lib/htlc.js `isHex`/`isHexOrMinima`/`isDecimal` + per-field guards; lib/order.js `fromJson` blanks mpk/eth | ✅ 0.1.20 |
| **Inbound OTC offer expiry** (native 0.1.43) — an inbound offer awaiting MY move (PROPOSED/COUNTERED + my turn) was exempt from expireStale, so an ignored offer sat forever while the proposer's side had already expired after 1h — a zombie deal showing a days-stale price as actionable. Now ages out after INBOUND_EXPIRE_MS (24h), then prunes on the normal terminal path | lib/otc.js INBOUND_EXPIRE_MS + expireStale my-turn branch; two expireStale tests in test/otc.test.js | ✅ 0.1.23 |
| **Role-aware refund reason + Minima-leg notify** (native 0.1.44) — the Minima-leg refund said only "Timelock passed"; refundReason claimed "never locked their side" even for a RESPONDER whose counter-leg exists because the counterparty DID lock (what failed was their claim — proven live 2026-08-27, first-time buyer, 438 USDT). Both legs now state the role-correct reason; EV_MISMATCH note still wins | lib/settle.js refundReason (getSwap role branch) + checkExpiredMinima notify | ✅ 0.1.24 |

| **History is scoped to the ACTIVE currency** (native 0.1.58) — AtomiX trades ONE currency at a time over a SHARED swap DB, so the Activity tab listed BOTH markets at once: switching to MINIMA still showed the dollar history. The rows always carried their market (baseSwap stamps `TradingContext.active().coinLabel` into selltoken/buytoken); the list just never read it. Present since the combine gave the app a second currency over currency-blind display reads. A swap's own market always shows it; the OTHER market only while it is still actionable (non-terminal AND inside its timelock, +1 day grace) — settlement is currency-agnostic, so an in-flight leg must never be hidden, but a long-dead row belongs in its own currency's history. Every unknown fails OPEN | lib/trading.js `forCoinLabel`/`forSwap` (one canonical label → market map, accepts the legacy `mxUSDT` spelling, never the bare `USDT`) + `visibleIn`/`withinWindow`; lib/ui.js activityTab filter; lib/app.js refreshSwaps carries mytimelock/mylegminima; lib/settle.js expectedTokenId reuses the map; test/visibility.test.js | ✅ 0.1.27 |

| **Market prints are tagged with their market** (native 0.1.59) — `market_trades` had no token column, so the Market-history tab mixed both currencies into one series: parity prints (~1.00) plotted alongside MINIMA's (~0.004) under an axis labelled with whichever currency was active. Collection was already token-scoped (`scanAllHtlcCoins` sends `tokenid:` + the active token); only the persisted rows and the display reads were blind. The same gap silently CORRUPTED data — the collector reconciles every OPEN row missing from the current scan as spent, but that scan is token-filtered, so after a switch the other market's open locks were absent by construction and got marked EXECUTED/REFUNDED wholesale | lib/swapdb.js `market_trades.tokenid` + `ALTER TABLE … ADD COLUMN IF NOT EXISTS` and a one-time price-band back-attribution (0.5 sits in the ~200x gap between the two markets; display only); `openTrades`/`recentTrades`/`executedTrades` take the token; lib/market.js stamps + scopes the reconcile; lib/app.js refreshMarketHistory passes the active token; test/market.test.js asserts both | ✅ 0.1.28 |

| **The recorded-transaction line is always printed** (native 0.1.61) — the missing-leg branch said "check the recorded transaction" and then printed nothing whenever no event carried a txpow id. That is exactly the phantom-row case: the swap row and secret are saved BEFORE the lock is broadcast (so a lost reply cannot strand a claimable leg), and only a successful post writes an id — so "no recorded transaction" is the evidence that nothing was posted, and the report was discarding it. Found live 2026-09-20 on a 7500 MINIMA leg the 1024-block lookup could not find | lib/inspect.js `recordedTxnSummary(count)`, appended unconditionally; the my-leg sentence now points at a line that always exists; test/safety.test.js covers both branches | ✅ 0.1.34 |
