...
| ⚠ **OPEN DEFECT (MDS + desktop only): the swap identity ROTATES every boot.** `htlc.js setup()` takes its identity from `getaddress`, which is core `Wallet.getDefaultAddress()` → `new Random().nextInt(numkeys)` — a RANDOM one of the node's default keys per call — and MDS persists it nowhere (`ctx.htlc.publickey`, in-memory only; no cfgSet). Native persists precisely to avoid this ("the published maker key must stay constant"). Consequence: after ANY service restart / page reload, a counter-leg locked to the PREVIOUS boot's key no longer satisfies `isMyPublishKey(stateAt(coin,4))` in settle.js's claim discovery, so it is never claimed → the swap sits in CLAIMING → the counterparty refunds at timeout and keeps the side you already paid. Same money-losing symptom as the 0.1.17 native incident, reached by a different route (rotation, not orphaning). The coin is NOT cryptographically lost — the node still owns that key, so a claim signed with `publickey:receiver` would work; it is purely a discovery-filter miss. **Fix (proposed, not yet applied): persist the identity like native (cfg row) + verify-and-heal it against loadKeys, AND widen claim discovery to match ANY node key rather than only the current publish key.** | ❌ open | ❌ |
