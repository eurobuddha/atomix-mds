 ...
 *   (2) a UX projection — `swaps` (one resumable card per swap).
 * (native's `market_trades` network-history layer — MarketCollector + chart — is NOT ported yet; when it is, its
 *  observer belongs beside the notify-secret harvest in settle.js, which already scans the same coins.)
...
(function (g) {
    ...
    AX.swapdb = {
        EV_STARTED: EV_STARTED, EV_CPSENT: EV_CPSENT, EV_COLLECT: EV_COLLECT, EV_EXPIRED: EV_EXPIRED,
