...
(function (g) {
    ...
    var EV_MINIMA_CLAIM_SUBMITTED = 'MINIMA_CLAIM_SUBMITTED', EV_MINIMA_REFUND_SUBMITTED = 'MINIMA_REFUND_SUBMITTED';
    // ---- UX swap status ----
    ...
    function init(cb) {
        var ddl = [
            ...
            "CREATE TABLE IF NOT EXISTS `market_trades` (`coinid` varchar(200) NOT NULL PRIMARY KEY, `hash` varchar(200), " +
                ...
                "`tokenid` varchar(200))",
            // Existing stores predate `tokenid` — add it, then back-attribute the untagged rows ONCE by price
            // band. A heuristic, but a wide one: the dollar market is pegged at parity (~1.00) while MINIMA
            // trades around 0.004, ~200x apart, so 0.5 sits in empty space between them. Display only — no
            // fund decision reads this column.
            "ALTER TABLE `market_trades` ADD COLUMN IF NOT EXISTS `tokenid` varchar(200)",
