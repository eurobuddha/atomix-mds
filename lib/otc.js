...
(function (g) {
    ...
    function initDb(cb) {
        var ddl = [
            ...
            // executes leg 1 of an AGREED deal — the atomic gate that stops a cross-instance double-lock.
            "CREATE TABLE IF NOT EXISTS `otc_exec` (`ref` varchar(80) NOT NULL PRIMARY KEY, `owner` varchar(80))"
        ];
        F.each(ddl, function (s, i, n) { M.sql(s, function (r) { n(sqlErr(r)); }); }, cb);
    }
    ...
    /** Atomically claim the right to execute leg 1 for `ref` — INSERT-if-absent, then read back the owner. Only the
    ...
    function claimExecute(ref, cb) {
        ensureToken(function (tok) {
            var r = esc(ref), t = esc(tok);
            M.sql("INSERT INTO `otc_exec` (`ref`,`owner`) SELECT '" + r + "','" + t + "' WHERE NOT EXISTS (SELECT 1 FROM `otc_exec` WHERE `ref`='" + r + "')", function () {
    ...
    /** Expire stale deals + prune long-finished ones. settleFn(hash, cb(swapStatus)) links to on-chain settlement. */
