...
(function (g) {
    ...
    var C = null, cpInFlight = {}, incoming = {};   // cpInFlight: hash → reserved coinids csv; incoming: hash → 1
    ...
    function acceptTakerSellMinima(order, coin) {
        ...
        var giveUsdt = num(H.stateAt(coin, 1));                    // USDT they requested, I'd pay
        if (recvMinima <= 0) return false;
    ...
    function onSellTake(coin, block, next) {
        if (!ready() || !myOrder()) return next();   // not a live maker → nothing to respond to
        ...
        if (String(H.stateAt(coin, 7)).toUpperCase() === 'TRUE') return next();   // OTC → Phase 6
        ...
        DB.hasEvent(hash, DB.EV_CPSENT, function (e, sent) {
            ...
            DB.getSwap(hash, function (e2, existing) {
                ...
                if (!acceptTakerSellMinima(myOrder(), coin)) return next();      // fund gate
                if (cpBurstFull(hash)) return next();
                reserveHash(hash, '');
    ...
    function lockEthCounterLeg(coin, hash, reqTokenAddr, next) {
        ...
        var sellRaw = D.parseUnits(tokenHuman, token.decimals), reqRaw = D.parseUnits(reqMinimaHuman, 18);
            ...
            ethChainNowStrict(function (eT, chainNow) {
                ...
                DB.upsertSwap(swap, function () {
                    ...
                    o.newContract(C.myMinimaPk, receiverEth, hash, BigInt(timelock), token.address, sellRaw, reqRaw, false, function (eN, txhash) {
                        if (eN) { releaseHash(hash); return next(); }
    ...
    function scanIncomingBuys(block, next) {
        if (!ready() || !C.myIdentity || !myOrder()) return next();   // not a live maker → don't scan the TAKE channel
            ...
            AX.mds.cmd('coins simplestate:true order:desc depth:' + SCAN_DEPTH + ' address:' + addr, function (r) {
                ...
                for (var i = 0; i < coins.length; i++) {
                    ...
                    try {
                        ...
                        if (msg && msg.hash && msg.to === C.myIdentity.publicId()) incoming[normHash(msg.hash)] = msg.hash;
                    } catch (e) { }
                }
                var hashes = [];
                for (var k in incoming) hashes.push(incoming[k]);
                F.each(hashes, function (hash, j, nx) { processIncomingBuy(hash, block, nx); }, next);
            });
        });
    }

    /** Find one announced buy's USDT lock by deterministic contractId, verify it's to me, run the responder path. */
    function processIncomingBuy(hash, block, next) {
        DB.getSwap(hash, function (e, existing) {
            ...
            DB.hasEvent(hash, DB.EV_CPSENT, function (e2, sent) {
                ...
                ops().getContract(EO.contractId(hash), function (e3, c) {
                    if (e3 || !c) return next();                              // USDT lock not visible yet → retry
                    ...
                    if (c.otc) return next();   // OTC take → only the agreed LP responds (Phase 6) — never the ladder responder
                    if (c.timelock - nowUnix() < Number(AX.htlc.CP_SECS_CHECK)) return next();   // their USDT lock too close to timeout
                    if (!acceptTakerBuyMinima(myOrder(), c)) return next();   // fund gate
                    if (cpBurstFull(hash)) return next();
    ...
    // ---- non-blocking allowance gate (F4 zero-first) ----
    var MAX_UINT = (1n << 256n) - 1n, APPROVE_TTL_MS = 5 * 60 * 1000, approvePending = {};
    function ensureAllowance(o, token, needed, cb) {
        o.allowance(token, function (e, cur) {
            ...
            var now = _now();
            if (cur > 0n) { var z = approvePending[token + '|0']; if (z == null || now - z > APPROVE_TTL_MS) { approvePending[token + '|0'] = now; return o.approve(token, 0n, function () { cb(null, false); }); } return cb(null, false); }
            var s = approvePending[token]; if (s == null || now - s > APPROVE_TTL_MS) { approvePending[token] = now; return o.approve(token, MAX_UINT, function () { cb(null, false); }); }
            cb(null, false);
        });
    }
    ...
    AX.responder = {
        ...
        onSellTake: onSellTake, scanIncomingBuys: scanIncomingBuys, processIncomingBuy: processIncomingBuy,
        _reset: function () { cpInFlight = {}; incoming = {}; approvePending = {}; }, _setNow: function (fn) { _now = fn; },
        _selectCoins: selectCoins, CP_LOCK_BURST: CP_LOCK_BURST
    };
