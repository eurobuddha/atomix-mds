 ...
 * The responder path (secret unknown → lock the counter-leg) is the MAKER's job → Phase 5, skipped here.
 *
...
(function (g) {
    ...
    var HTLC_SCAN_DEPTH = 256, ETH_RETRY_SECS = 150;
    ...
    function poll(cb) {
        ...
        H.currentBlock(function (e, block) {
            if (e) return cb();
            runMinimaChecks(block, function () { runEthChecks(block, cb); });
        });
    }
    ...
    function checkCanSwapCoin(coin, block, next) {
        ...
        DB.getSecret(hash, function (e, secret) {
            if (e || !secret) return next();               // no secret → responder path (Phase 5), not the taker
            DB.getRequest(hash, function (e2, req) {
                if (req && !amountTokenOk([req.reqAmount, req.reqToken], H.coinAmount(coin), reqTokenAddr, false)) {
                    return DB.logEvent(hash, DB.EV_COLLECT, 'minima', '0', 'counterparty amount/token mismatch', function () { next(); });
                }
                    ...
                    DB.getRequest(hash, function (e3, req) {
                        if (req) {
                            ...
                            if (!amountTokenOk([req.reqAmount, req.reqToken], gotHuman, gc.tokenContract, true)) {
                                return DB.logEvent(hash, DB.EV_COLLECT, 'ETH:' + gc.tokenContract, '0', 'counterparty amount/token mismatch', function () { next(); });
                            }
                        }
    ...
    function broadcastEthWithdraw(cid, hash, secret, next) {
        ...
        DB.setSwapStatus(hash, DB.ST_CLAIMING, function () {
            onChanged();
            ops().withdraw(cid, secret, function () { delete inflight[key]; next(); });   // success confirmed on-chain, not here
        });
    }
    function broadcastEthRefund(cid, hash, next) {
        ...
        if (!ethRetryDue(key) || inflight[key]) return next();
        inflight[key] = true; markEthAttempt(key);
        ops().refund(cid, function () { delete inflight[key]; next(); });
    }
