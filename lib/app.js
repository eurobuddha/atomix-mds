...
(function (g) {
    ...
    function refreshMarketHistory() {
        ...
        var tok = TR.active().tokenId;
        AX.swapdb.executedTrades(200, tok, function (e, chart) {
            AX.swapdb.recentTrades(50, tok, function (e2, recent) {
    ...
    function onSwapDetail(hash) {
        ...
        DB.getSwap(hash, function (e, s) {
            ...
            H.currentBlock(function (eB, block) {
                if (eB) block = -1;
                // DEEP: a swap worth inspecting is usually OLD — the shallow walk-back reported fully-locked
                // coins as "not found" (native parity: atomix 0.1.17).
                H.scanByHashDeep(hash, 2, 1024, function (eS, coins) {
                    ...
                    DB.getSecret(hash, function (e2, secret) {
                        DB.getEvents(hash, function (e3, events) {
                            ...
                            var facts = { swap: s, block: block, secretKnown: !!secret, myMin: myMin, cpMin: cpMin,
                                minimaError: eS ? eS.message : null, gc: null, gcAmountHuman: '', myEthStillLocked: null, events: events || [] };
                            function report() { UI.swapReportDialog(hash, AX.inspect.buildReport(facts)); }
                            // Keyed on whose leg is on which chain, NOT direction: direction names the INITIATOR's
                            // flow, so a RESPONDER row would read my OWN ETH lock as the "counterparty leg".
                            if (s.myLegIsMinima) {
                                ops.getContract(EO.contractId(hash), function (e4, gc) {
                                    facts.ethError = e4 ? e4.message : null;
                                    if (gc) { facts.gc = gc; facts.gcAmountHuman = DEC.formatUnits(gc.amount, usdtDp(gc.tokenContract)); }
                                    report();
                                });
                            } else {
                                ops.canCollect(s.contractId, function (e4, still) { facts.myEthStillLocked = e4 ? null : !!still; report(); });
                            }
                        });
                    });
                });
    ...
    function axCcyLabel(s) { var c = TR.forSwap(s.sellToken, s.buyToken); return c ? c.coinLabel : ''; }
    ...
    function axDirLabel(s) {
        var c = TR.forSwap(s.sellToken, s.buyToken), coin = c ? c.coinLabel : 'MINIMA', d = s.direction;
        if (d === 'MINIMA_TO_ERC20') return 'Sell ' + coin;
        if (d === 'ERC20_TO_MINIMA') return 'Buy ' + coin;
        return d || '';
    }
    ...
    function axCsvRow(s, ev) {
        return [axIso(s.created), axRoleLabel(s.role), axDirLabel(s),
            ...
            axPickTx(ev, true), axPickTx(ev, false), axCcyLabel(s)].map(axCsvCell).join(',');
    }
    function onExportCsv() {
        AX.swapdb.allSwaps(function (err, all) {
            if (err || !all || !all.length) { toast('No swaps to export'); return; }
            // Currency is APPENDED, never inserted, and no existing header is renamed: the reconciliation ledger
            // (tools/dexHistory/atomix_ledger.py) reads these columns BY NAME, "Price (USDT/MINIMA)" included.
            var lines = ['Date,Role,Direction,Sold Amount,Sold Token,Bought Amount,Bought Token,Price (USDT/MINIMA),Counterparty,Status,Contract Id,Minima Tx,Eth Tx,Currency'];
            var i = 0;
            (function next() {
    ...
    function onCoinDump() {
        AX.htlc.myRelevantCoins(TR.active().tokenId, function (err, resp) {
        ...
        M.sql('SELECT * FROM swaps ORDER BY created DESC', function (r) {
            if (r && r.status && r.rows) {
                UI.state.swaps = r.rows.map(function (row) {
                    // mytimelock + mylegminima are carried for the history scoping rule (TR.visibleIn): they are
                    // what decides whether an OTHER-currency swap is still actionable enough to pierce the filter.
                    return { hash: row.HASH, role: row.ROLE, direction: row.DIRECTION, selltoken: row.SELLTOKEN,
                        ...
                        mytimelock: row.MYTIMELOCK, mylegminima: row.MYLEGMINIMA };
                });
                UI.render();
            }
        });
    ...
    function switchCurrency(target) {
        ...
        AX.maker.onCurrencySwitch(makerBalances(), function (err) {
            // A failed tombstone or a failed kv write ABORTS the switch, loudly. Proceeding regardless (as this
            // did) left either the old ladder live and mispriced under the new currency, or a split brain: the
            // page trading as the new currency while the kv, service.js and the next boot stayed on the old.
            if (err) { toast('Currency switch aborted — could not withdraw the current market: ' + (err.message || err)); UI.render(); return; }
            ...
            M.kvSet('trading_currency', target.key, function (e2) {
                if (e2) { toast('Currency switch aborted — could not save the setting: ' + (e2.message || e2) + '. Your market is withdrawn; switch again.'); UI.render(); return; }
                TR.setActive(target);
