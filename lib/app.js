...
(function (g) {
    ...
    function refreshEthBalances() {
        ...
        rpc.getBalance(addr, function (e, wei) { if (!e) { UI.state.balsRaw.ethWei = wei.toString(); armPulse('eth', UI.state.bals.eth, trim6(DEC.formatUnits(wei, 18))); UI.state.bals.eth = trim6(DEC.formatUnits(wei, 18)); UI.render(); } });
        EO.make(rpc, UI.state.ctx.eth.privKey, addr).balanceOf(EO.NET.usdt, function (e, raw) { if (!e) { UI.state.balsRaw.usdtRaw = raw.toString(); armPulse('usdt', UI.state.bals.usdt, DEC.formatUnits(raw, 6)); UI.state.bals.usdt = DEC.formatUnits(raw, 6); UI.render(); } });
    }
    ...
    function switchCurrency(target) {
        ...
        AX.maker.onCurrencySwitch(makerBalances(), function () {
            M.kvSet('trading_currency', target.key, function () {
                ...
                AX.maker.loadConfig(function () { var st = AX.maker._state(); UI.state.makerCfg = st.cfg; UI.state.makerManual = st.manual; UI.render(); refreshBalances(); refreshEthBalances(); refreshBook(); });
            });
        });
    }
