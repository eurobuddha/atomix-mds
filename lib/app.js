...
(function (g) {
    ...
    function onBooted(ctx) {
            ...
            UI.onRefreshBook = refreshBook; UI.onEnterBookTab = refreshBook;
    ...
    function refreshEthBalances() {
        ...
        rpc.getBalance(addr, function (e, wei) { if (!e) { UI.state.balsRaw.ethWei = wei.toString(); UI.state.bals.eth = trim6(DEC.formatUnits(wei, 18)); UI.render(); } });
        EO.make(rpc, UI.state.ctx.eth.privKey, addr).balanceOf(EO.NET.usdt, function (e, raw) { if (!e) { UI.state.balsRaw.usdtRaw = raw.toString(); UI.state.bals.usdt = DEC.formatUnits(raw, 6); UI.render(); } });
    }
