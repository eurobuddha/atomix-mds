...
(function (g) {
    ...
    function render() {
        ...
        if (!S.paired && S.bootError) root.appendChild(bootErrorCard());   // boot failed → instructions overlay
    ...
    function walletTab() {
        ...
        return el('div', {}, [
            walletCard('Minima · available to swap', S.bals.minima + ' ' + ccy(), 'var(--accent)', minimaBreakdown(),
                function () { AX.ui.onCoinDump && AX.ui.onCoinDump(); }),
            walletCard('Ethereum · Ethereum', S.bals.eth + ' ETH', 'var(--text)',
                addr ? AX.wallet.shortAddr(addr) : 'deriving from node seed…', addr ? receiveDialog : null),
            walletCard('USDT · Ethereum', S.bals.usdt + ' USDT', 'var(--text)', null, null),
    ...
    function walletCard(title, val, color, sub, onclick) {
        var kids = [
    ...
    AX.ui = {
        state: S, dark: true, slip: 4.2, version: '0.1.6', block: 0,
