    ...
    function startSell(maker, sellMinima, symbol, buyTokenAmount, otc, cb) {
        ...
        H.generateSecret(function (e, sec) {
            ...
            H.currentBlock(function (e2, block) {
                ...
                var swap = baseSwap(sec.hash, 'INITIATOR', 'MINIMA_TO_ERC20', actLabel, sellMinima, symbol, buyTokenAmount, maker.ethAddress, timelock, true, null);
                recordStart(sec.hash, sec.secret, buyTokenAmount, reqToken, swap, function (eRec) {
                    ...
                    H.lock({
                        ...
                        receiverPubkey: maker.minimaPublicKey, ownerEthKey: C.ethAddr, hashlock: sec.hash,
        ...
        ensureAllowance(o, token.address, sellRaw, function (eAllow, ok) {
            ...
            H.generateSecret(function (e, sec) {
                ...
                ethChainNow(function (now) {
                    ...
                    var swap = baseSwap(sec.hash, 'INITIATOR', 'ERC20_TO_MINIMA', symbol, sellTokenAmount, actLabel, reqMinima, maker.minimaPublicKey, timelock, false, EO.contractId(sec.hash));
                    recordStart(sec.hash, sec.secret, reqMinima, 'minima', swap, function (eRec) {
                        if (eRec) return cb(eRec);
                        notifyChanged();
                        o.newContract(C.myMinimaPk, maker.ethAddress, sec.hash, BigInt(timelock), token.address, sellRaw, reqRaw, !!otc, function (eNc, txhash) {
    ...
    function startLeg(maker, symbol, sellMinima, minima, usdtAmt, hooks, cb) {
        ...
        if (sellMinima) {
            ...
            startBuy(maker, symbol, usdtAmt, minima, false, function (e, hash) {
                ...
                if (!hooks.me || !maker.commsPublicId) { if (hooks.onNote) hooks.onNote('buy-started-no-contact'); return; }
                var myPub = hooks.myPublicId || hooks.me.publicId();   // keep the sealed "from" consistent with the sender
                AX.take.send(hooks.me, maker.commsPublicId, myPub, hash, function (te) {
