...
(function (g) {
    ...
    /** Split my own coins into `count` equal coins totalling `totalAmount` (CONFIRMED inputs) so a ladder has enough
     *  separately-spendable coins to lock every sweep leg concurrently. cb(err, txpowid). */
    function splitCoins(count, totalAmount, tokenId, myAddress, cb) {
        M.cmdR('send amount:' + totalAmount + ' address:' + myAddress + ' tokenid:' + tokenId + ' split:' + count + ' coinage:1 mine:true',
            function (err, resp) { cb(err, err ? '' : txpowOf(resp)); });
    }
    ...
    AX.htlc = {
        ...
        coinAmount: coinAmount, lock: lock, lockFromCoins: lockFromCoins, myFreeCoins: myFreeCoins, splitCoins: splitCoins,
